...
impl AgentDriver for TmuxDriver {
    ...
    async fn stream_chat(
        ...
        if !self.is_running().await {
            ...
            let mut start_cmd = self.wrap_command(
                "tmux",
                &[
                    "new-session",
                    "-d",
            ...
            let output = start_cmd.output().await.map_err(|e| {
                AgentError::Driver(format!("[{}] Failed to start tmux: {}", self.name, e))
            })?;
        ...
        if let Err(e) = self
            .wrap_command(
                "tmux",
                &["send-keys", "-t", &self.config.session_name, "-l", &prompt],
            )
            .status()
            .await
        {
        ...
        if let Err(e) = self
            .wrap_command(
                "tmux",
                &["send-keys", "-t", &self.config.session_name, "Enter"],
            )
            .status()
            .await
        {
        ...
        if let Some(log_cmd_str) = &self.config.log_command {
            ...
            if parts.is_empty() {
                return Err(AgentError::Driver("Empty log_command".into()));
            }
            ...
            let _driver_name = self.name.clone();
                                ...
                                serde_json::from_str::<serde_json::Value>(&line)
                            {
                                if let Some(filter) = &config_clone.output_filter {
                                    ...
                                    for part in filter.split('.') {
                                        path_val =
                                            path_val.get(part).unwrap_or(&serde_json::Value::Null);
                                    }
    ...
    async fn stop(&self) -> Result<()> {
        if self.is_running().await {
            let mut cmd =
                self.wrap_command("tmux", &["kill-session", "-t", &self.config.session_name]);

...
use ch_protocol::{AgentAddress, AgentId, AgentMessage, EvidenceVerifyMsg, MessageType, Payload};
use std::sync::Arc;
...
impl VerifierRule for KeywordRule {
    ...
    async fn evaluate(&self, row: &EvidenceRow) -> RuleOutcome {
        if row.claim.contains("__test_pass__") {
            ...
            RuleOutcome::Fail {
                reason: "test sentinel: __test_fail__".to_string(),
            }
        } else {
            RuleOutcome::Skip
        }
...
pub fn spawn_verifier(
    ...
    tokio::spawn(async move {
        ...
        info!(
            "verifier: started with {} rule(s), polling every {}s",
            rules.len(),
...
async fn emit_verdict(
    ...
    let from = AgentAddress {
        ...
        agent_name: format!("verifier:{}", rule_name),
        adapter_type: "verifier".into(),
    };
